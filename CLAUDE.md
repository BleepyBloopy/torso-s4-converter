...
Table column layout: `["Open Folder", "▶", "Path", "Type", "File / Folder", "Size", "Detail / Edit"]`. Columns 1 (Open Folder) and 2 (▶ play button) are `Fixed` resize mode at 36px each — the `_rebuild_display` auto-resize loop skips Fixed-mode columns to preserve their widths. The play (▶/⏹) and open-folder (📂) cell widgets are placed by `_add_row_widgets()`, registered as `table._post_render_fn` so they are re-created on every page navigation. `_toggle_preview(btn, path)` delegates to `main_window._player` (a shared `QMediaPlayer`); `main_window._playing_btn` tracks the currently-active button so page navigation can update its icon. `apply_phase_2` (Long Prefix) always re-reads the folder live at apply time so files already renamed by BPM Relabel in the same session are picked up correctly.
